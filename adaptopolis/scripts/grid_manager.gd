...
func _generate_buildings() -> void:
    var building_count := rng.randi_range(2, 3)
...
func _resolve_merges(facility: Facility) -> void:
    var neighbors := _get_neighbor_facilities(facility)
    for neighbor in neighbors:
        ...
        if neighbor.level != facility.level:
            continue
        var absorbed_cells := facility_cells.get(neighbor, []).duplicate()
...
func remove_facility(facility: Facility) -> void:
    if not facility_cells.has(facility):
        return
    var positions: Array[Vector2i] = facility_cells[facility]
...
func _get_neighbor_facilities(facility: Facility) -> Array:
    var neighbors: Array = []
    var seen: Dictionary = {}
    var directions = [Vector2i.UP, Vector2i.DOWN, Vector2i.LEFT, Vector2i.RIGHT]
    var positions := facility_cells.get(facility, [])
    for pos in positions:
        for dir in directions:
            ...
            if neighbor_cell == null:
                continue
            var neighbor_facility := neighbor_cell.facility_ref
...
func load_state(data: Array, library: FacilityLibrary, building_positions: Array = []) -> void:
    ...
    for entry in data:
        ...
        var origin := Vector2i(origin_array[0], origin_array[1])
